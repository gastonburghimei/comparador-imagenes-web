...
CREATE or replace TABLE `meli-bi-data.SBOX_PFFINTECHATO.consolidado_tmp_GB` as
    SELECT 
        ...
        ,face_val_30m
        ,tipo_login
    FROM `meli-bi-data.SBOX_PFFINTECHATO.pyt_temp_tmp_GB`
    UNION ALL 
    SELECT 
    ...
    FROM `meli-bi-data.SBOX_PFFINTECHATO.withdrawals_temp_tmp_GB`
    UNION ALL 
    SELECT  
        ...
        */
        ...
        ,config_id_ctx,flow_type_ctx,ref_id_nw_dev,device_id,device_type,device_creation_date_all_users, device_creation_date,ip,
        face_val_30m, tipo_login
    FROM `meli-bi-data.SBOX_PFFINTECHATO.pay_temp_tmp_GB`
;
